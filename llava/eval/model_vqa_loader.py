...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in data_bar:
        ...
        from llava.model.llava_arch import erank_log
        cur_erank = erank_log[-1] if erank_log else None
        ...
        ans_file.write(json.dumps({"question_id": idx,
                                   ...
                                   "answer_id": ans_id,
                                   "model_id": model_name,
                                   "metadata": {"erank": cur_erank}}) + "\n")
        # ans_file.flush()
    ans_file.close()
    avg_token_num = sum(visual_token_nums) / len(visual_token_nums)
    print(f"Average number of visual tokens: {avg_token_num}")
    from llava.model.llava_arch import n_rank, n_sample
    if n_sample > 0:
        print(f"Average erank of selected tokens: {n_rank / n_sample:.4f}")
